...
def retail_source(raw_path: str = "data_raw"):
    ...
    def load_customers():
        ...
        # Apply scale to limit rows
        row_limit = int(len(customersdf) * scale)
        customersdf = customersdf.head(row_limit)
        print(f"Processing {row_limit} customer records (scale={scale})")
